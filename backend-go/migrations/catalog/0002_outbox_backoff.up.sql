...
ALTER TABLE catalog.outbox_messages ADD COLUMN IF NOT EXISTS next_attempt_at timestamptz;

-- Bekleyen mesaj taraması için kısmi indeks (işlenmişler indekse girmez).
CREATE INDEX IF NOT EXISTS ix_catalog_outbox_pending
    ON catalog.outbox_messages (occurred_on_utc)
