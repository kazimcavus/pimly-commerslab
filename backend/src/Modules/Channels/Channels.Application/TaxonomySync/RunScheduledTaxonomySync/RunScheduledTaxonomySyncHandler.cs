...
using Channels.Application.TaxonomySync.Scheduling;
    ...
    ILogger<RunScheduledTaxonomySyncHandler> logger) : IRunScheduledTaxonomySyncHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var enqueuedCount = 0;
        foreach (var marketplace in Marketplace.AllSupported)
        {
            if (await syncRuns.HasSyncRunSinceAsync(marketplace, slotStart, cancellationToken))
            {
                continue;
            }

            var enqueueResult = await enqueueTaxonomySync.ExecuteAsync(
                new EnqueueTaxonomySyncCommand(marketplace),
                cancellationToken);
            ...
            if (enqueueResult.IsFailure)
            {
                ...
                {
                    logger.LogWarning(
                        "Scheduled taxonomy sync skipped for marketplace {Marketplace}: {ErrorMessage}",
                        marketplace.Code,
            ...
            if (logger.IsEnabled(LogLevel.Information))
            {
                logger.LogInformation(
                    "Scheduled taxonomy sync enqueued for marketplace {Marketplace} in slot starting {SlotStartUtc}. SyncRunId={SyncRunId}",
                    marketplace.Code,
                    slotStart,
