    ...
    ITaxonomySyncRunRepository syncRuns) : IGetTaxonomySyncRunHandler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Result.Failure<TaxonomySyncRunDto>(keyResult.Error);
        }
        ...
        if (syncRun is null || syncRun.MarketplaceKey != marketplaceResult.Value.Key)
        {
            return Result.Failure<TaxonomySyncRunDto>(Error.NotFound("Taxonomy sync run not found."));
        }

        return Result.Success(syncRun.ToDto(marketplaceResult.Value.Key));
    }
}
