...
/// <summary>Inventory EF modeline tenant shadow property ve query filter ekler.</summary>
internal static class InventoryTenancyExtensions
{
    public static void ApplyInventoryTenancy(this ModelBuilder modelBuilder, Guid tenantId)
    {
        ConfigureTenantRoot<StockLevel>(modelBuilder, tenantId);
    }
        ...
        foreach (var entry in db.ChangeTracker.Entries())
        {
            ...
            if (tenantId == Guid.Empty)
            {
                throw new InvalidOperationException(
                    "Tenant id is required to persist tenant-scoped inventory data.");
            }
